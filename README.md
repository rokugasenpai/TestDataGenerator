...
前・後処理(マスタDB取込など)のみ実行することもできます。  
その場合は、mainメソッドは実行せずに、前処理はexecute_pre_procメソッドを、  
後処理はexecute_post_procメソッドを実行します。  
設定ファイルは./config/json/前処理と後処理のみ実行.jsonを参考にしてください。
...
- record_rules
    - レコードルール
    - データを生成する場合は必須(前・後処理のみ実行する場合は不要)
...
- sql
    - SQL
    - DBよりマスタを利用してデータ生成する場合は必須(前・後処理のみ実行する場合は不要)
    - 文字列
