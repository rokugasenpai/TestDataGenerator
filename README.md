...
- tests
    ...
    - operation
        - 運用テストのテストケースが入っています。  
        2000件のUsersテーブルを想定したテストデータ出力のテストができます。  
    - stress
        - 耐久テストのテストケースが入っています。  
        100000件のUsersテーブルを想定したテストデータ出力のテストができます。  
        xdebugが有効の場合、当方の環境では3倍ほど遅くなることが確認できたため、  
        純粋なベンチマークとしての使用はお勧めしません。  
    - unit
        - 単体テストのテストケースが入っています。  
...
## テスト結果
単体・機能・運用・耐久の全テストで正常に通過しています。  
今後アップデートの度に、テストを走らせ、ここに結果を書くのも大変なので、  
近いうちにPHPUnitのテスト体系見直し、Travisの導入、VPSにJenkinsを構築しようと思います。  
...
- CentOS6.7 PHP5.6 CPU2.6GHz3コア メモリ2GB SSD VPS
    ...
    - Usersテーブルテストデータ100000件
        - xdebug有効
            - 8時間以上経過したため打ち切り  
            検証していませんが、VPSなため高負荷による制限がかけられた可能性があります。  
        - xdebug無効
            - 未計測
...
- Windows7 PHP5.4 CPU3.3GHz2コア メモリ4GB HDD 自機
    ...
    - Usersテーブルテストデータ100000件
        ...
        - xdebug無効
            - 約42分
            - (終了テストデータ作成日時) - (開始＝時計目視)  
            による概算なので、時刻・時間が表示できるようプログラムの改修を行い再計測します。  

## ToDo
- PHPUnitのテスト体系見直しとTravisの導入を行います。  
全テスト実行されるテストスイートを設定します。

- 設定項目output_memory_limitを削り、memory_limitを入れます。  
output_memory_limitに対応する値は、memory_limit / 8とする予定です。  

- 開始日時・終了日時・経過時間が標準出力できるように改修し、  
xdebugなし(カバレッジなし)のベンチマークを行います。  

- 出力するテストデータ件数が増えるほど、メモリ使用量が増える設計となっているため、  
1レコード毎ファイル出力し全レコードを保持しない設定項目を追加します。  
ベンチマークも併せて行います。  
